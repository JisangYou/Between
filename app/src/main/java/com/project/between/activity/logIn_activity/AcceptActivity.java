...
public class AcceptActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myNumber = intent.getStringExtra(ConstantUtil.MY_PHONE_NUMBER);
        friendNumber = intent.getStringExtra(ConstantUtil.FRIEND_PHONE_NUMBER);
        tempkey = intent.getStringExtra(ConstantUtil.TEMP_KEY2);

        database = FirebaseDatabase.getInstance();
        userRef = database.getReference(ConstantUtil.USER);
        myNumRef = database.getReference(ConstantUtil.TEMP_CHECK).child(myNumber).child("status");
